...
const RutaPanel = () => {
  ...
  /* ── Podcast Helpers ── */
  const startPodcast = () => {
    setPodcastActive(true); setPodcastSegment(0); setPodcastExerciseIdx(0);
    setPodcastUserAnswer(""); setPodcastFeedback(null);
  ...
  const checkPodcastAnswer = (submitted = null) => {
    ...
    const lang = (ex.type === "translateES" || ex.type === "choose") ? "es" : "de";
    const result = window.Corrector.check(ans, ex.answer, lang);
    setPodcastFeedback({ correct: result.correct, exact: result.exact, answer: ex.answer, hint: result.message });
  };
  const nextPodcastExercise = () => {
    const seg = window.PodcastA1_1?.segments[podcastSegment];
    if (!seg) return;
    if (podcastExerciseIdx < seg.exercises.length - 1) {
      setPodcastExerciseIdx(podcastExerciseIdx + 1); setPodcastUserAnswer(""); setPodcastFeedback(null);
    } else {
      const nextSeg = podcastSegment + 1;
      if (nextSeg < window.PodcastA1_1?.segments.length) {
        setPodcastSegment(nextSeg); setPodcastExerciseIdx(0); setPodcastUserAnswer(""); setPodcastFeedback(null);
        const newSeg = window.PodcastA1_1?.segments[nextSeg];
        if (newSeg?.audioText) speak(newSeg.audioText);
      } else {
        setPodcastActive(false); setPodcastSegment(0); setPodcastExerciseIdx(0);
        setPodcastUserAnswer(""); setPodcastFeedback(null);
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 p-4">
        <div className="max-w-2xl mx-auto">
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={goBack} className="px-4 py-2 bg-slate-700 text-slate-200 rounded-lg hover:bg-slate-600 transition shadow">← Volver</button>
          </div>
          <div className="bg-slate-800 p-8 rounded-2xl shadow-2xl mb-4 border border-slate-700">
            <div className="flex items-center mb-6">
              <p className="text-slate-200 font-medium text-lg">{ex.prompt}</p>
              <button onClick={(e) => { e.stopPropagation(); speak(ex.answer?.split(' ')[0]?.startsWith('der') || ex.answer?.split(' ')[0]?.startsWith('die') || ex.answer?.split(' ')[0]?.startsWith('das') ? ex.answer : ex.prompt); }} className="ml-2 text-slate-400 hover:text-white transition" title="Escuchar">🔊</button>
            </div>
            {ex.options && ex.type !== "matchPairs" ? (
              <div className="space-y-3">
                {ex.options.map((opt, i) => (
                    ...
                    <button key={idx} onClick={() => {
                      ...
                      if (!matchSelected) { setMatchSelected({ idx, word }); }
                        ...
                        if (correctPair) {
                          ...
                          if (matchResult.length + 1 === ex.pairs.length) checkAnswer(null);
            ...
            <div key={level.id} className={`bg-slate-800 rounded-2xl shadow-xl p-5 mb-4 border border-slate-700 transition ${!isUnlocked ? "opacity-40" : "hover:border-blue-500"}`}>
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex items-center">
                  ...
                  {level.id === "A1.1" && (<button onClick={startStory} className="ml-2 px-3 py-1 bg-pink-600 text-white text-xs rounded-full hover:bg-pink-500 transition shadow">🎬 Historia</button>)}
                  {level.id === "A1.1" && (<button onClick={startPodcast} className="ml-2 px-3 py-1 bg-purple-600 text-white text-xs rounded-full hover:bg-purple-500 transition shadow">🎙️ Podcast</button>)}
                </div>
              </div>
