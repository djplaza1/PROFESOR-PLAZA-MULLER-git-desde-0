...
const PodcastView = ({ levelId, lessonIdx, onBack }) => {
  ...
  const check = (submitted = null) => {
    ...
    setFeedback({ translation: seg.translation, correct: result.correct, exact: result.exact, answer: ex.answer, hint: result.message });
  };
  const next = () => {
  ...
  if (!ex) return React.createElement("div", { className: "text-white p-4" }, "Cargando podcast...");
  return React.createElement("div", { className: "min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 p-4" },
    React.createElement("div", { className: "max-w-2xl mx-auto" },
      React.createElement("div", { className: "flex justify-between items-center mb-4" },
        React.createElement("h2", { className: "text-xl font-bold text-white" }, `🎙️ Podcast – Segmento ${segment+1}/${totalSegments}`),
      ...
      feedback && React.createElement("div", { className: `p-4 rounded-xl ${feedback.correct ? "bg-emerald-900/80 text-emerald-200 border border-emerald-700" : "bg-red-900/80 text-red-200 border border-red-700"}` },
        ...
        : React.createElement("div", null,
            React.createElement("span", null, "❌ Incorrecto. La respuesta correcta es: "),
            React.createElement("strong", { className: "text-white" }, feedback.answer)
          )
      ),
