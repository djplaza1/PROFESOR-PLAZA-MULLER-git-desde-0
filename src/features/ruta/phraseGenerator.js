const PhraseGenerator = {
  ...
  // Seleccionar elementos aleatorios de un array excluyendo uno
  ...
  // Obtener todas las palabras de un idioma desde el vocabulario del nivel (o global)
  getWordsInLang(levelId, langIndex) {
    const R = (window.Muller && window.Muller.Ruta) ? window.Muller.Ruta : null;
    if (!R || !R.VOCAB) return [];
    // Si el nivel tiene vocabulario, lo usamos; si no, todo el vocabulario
    const levelVocab = R.VOCAB[levelId] || [];
    if (levelVocab.length < 5) {
      // Combinar todos los niveles para tener suficiente variedad
      let all = [];
  ...
  generateExercises(levelId, lessonIdx, wordsPerLesson) {
    ...
    // Pre‑obtener listas de palabras en alemán y español para los distractores
    ...
    words.forEach((word, i) => {
      ...
      switch(typeIdx) {
        case 0: // fill: completar palabra alemana
