...
// ---------------------------------------------------
// Panel de escritura con 10 modos (libre, copia, dictado, prompt, TELC, letras, guion, vocab, mecanografÃƒÂ­a, manuscrito)
// ---------------------------------------------------
...
window.Muller.Panels.EscrituraPanel = function EscrituraPanel({ session }) {
  ...
  const DE_LETTERS = ['Ãƒâ€ž','Ãƒâ€“','ÃƒÅ“','ÃƒÅ¸'];
  ...
  const getPos = (e) => {
    const rect = canvasRef, viewerRef.current.getBoundingClientRect();
  ...
  const handlePointerDown = (e) => {
    ...
    canvasRef, viewerRef.current.setPointerCapture(e.pointerId);
  };
  ...
  const handlePointerMove = (e) => {
    ...
    const ctx = canvasRef, viewerRef.current.getContext('2d');
    ctx.beginPath();
  ...
  const handlePointerUp = (e) => {
    ...
    try { canvasRef, viewerRef.current.releasePointerCapture(e.pointerId); } catch(ex) {}
  };

  const undoStroke = () => { strokes.current.pop(); redraw(); };
  const clearCanvas = () => { strokes.current = []; redraw(); };
  const saveCanvasImage = () => {
    const dataUrl = canvasRef, viewerRef.current.toDataURL('image/png');
  ...
  const triggerOcr = () => {
    if (window.Muller.runOcrOnCanvas) {
      window.Muller.runOcrOnCanvas(canvasRef, viewerRef.current).then(result => {
  ...
  // Efectos de inicializaciÃƒÂ³n para typing y handwrite
  useEffect(() => {
  ...
  // Actualizar WPM y precisiÃƒÂ³n en tiempo real para modo typing
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const currentTypingText = (typingPool[typingIdx % typingPool.length] || {}).text || '';
    const ratio = currentTypingText.length > 0 ? Math.min(typingInput.length / currentTypingText.length, 1) : 0;
  ...
  return React.createElement('div', {
    ...
    React.createElement('p', { className: 'text-stone-300/95 text-xs md:text-sm mb-4 leading-relaxed border-b border-white/10 pb-3' },
      'Zona solo para escribir a mano Ã¢â‚¬â€ pensada para ',
      React.createElement('strong', { className: 'text-white' }, 'tableta con lÃƒÂ¡piz'),
      '. El lienzo usa ',
      React.createElement('strong', { className: 'text-white' }, 'pointer capture'),
      '.',
    ),

    // Selector de modos (10 botones)
    React.createElement('div', { className: 'flex flex-wrap gap-1.5 md:gap-2 mb-4' },
      [
        { id: 'free', label: 'Libre', sub: 'notas / borrador' },
        { id: 'copy', label: 'Copia', sub: 'caligrafÃƒÂ­a' },
        { id: 'dictation', label: 'Dictado', sub: 'oÃƒÂ­r y escribir' },
        { id: 'prompt', label: 'Tema', sub: 'redacciÃƒÂ³n' },
        { id: 'telc', label: 'TELC', sub: 'carta/email examen' },
        { id: 'letters', label: 'Letras DE', sub: 'Ãƒâ€žÃƒâ€“ÃƒÅ“ÃƒÅ¸' },
        { id: 'guion', label: 'Guion', sub: 'misma historia' },
        { id: 'vocab', label: 'Palabra', sub: 'del vocab' },
        { id: 'typing', label: 'Mecano', sub: 'teclado' },
        { id: 'handwrite', label: 'Manuscrito', sub: 'lÃƒÂ¡piz ÃƒÂ³ptico' }
      ].map(m =>
    ...
    spellErrors.length > 0 && React.createElement('div', { className: 'mt-4 rounded-xl bg-black/35 border border-amber-500/30 p-3' },
      React.createElement('p', { className: 'text-amber-200/90 text-sm font-bold mb-2' }, '?? Resultados de correcciÃƒÂ³n'),
    ...
    telcCoachResult && React.createElement('div', { className: 'mt-4 rounded-xl bg-black/35 border border-emerald-500/30 p-3 space-y-2' },
      ...
      React.createElement('div', { className: 'grid grid-cols-2 md:grid-cols-4 gap-2' },
        [
          ...
          { label: 'CohesiÃƒÂ³n', score: telcCoachResult.scoreCohesion },
          { label: 'GramÃƒÂ¡tica', score: telcCoachResult.scoreGrammar }
        ].map(item =>
...
// --- FunciÃƒÂ³n helper para renderizar el contenido de cada modo ---
  ...
  switch (mode) {
    case 'free':
      return React.createElement('div', { className: 'mb-4 rounded-xl bg-black/35 border border-rose-500/25 p-3' },
        React.createElement('p', { className: 'text-rose-200/90 text-sm font-bold' }, 'PÃƒÂ¡gina en blanco'),
    ...
    case 'copy':
      return React.createElement('div', { className: 'mb-4 rounded-xl bg-black/35 border border-rose-500/25 p-3 space-y-2' },
        React.createElement('p', { className: 'text-rose-200/90 text-sm font-bold' }, 'Copia la frase (caligrafÃƒÂ­a alemana)'),
    ...
    case 'dictation':
      return React.createElement('div', { className: 'mb-4 rounded-xl bg-black/35 border border-rose-500/25 p-3 space-y-3' },
        React.createElement('p', { className: 'text-rose-200/90 text-sm font-bold' }, 'Dictado alemÃƒÂ¡n'),
        ...
        React.createElement('p', { className: 'text-[10px]' }, `ÃƒÂtem ${Math.min(writingDictIdx+1, writingDictationPool.length)} de ${writingDictationPool.length}`),
    ...
    case 'prompt':
      return React.createElement('div', { className: 'mb-4 rounded-xl bg-black/35 border border-rose-500/25 p-3 space-y-2' },
        React.createElement('p', { className: 'text-rose-200/90 text-sm font-bold' }, 'Tema para redacciÃƒÂ³n corta'),
        ...
        WRITING_TELC_TASKS.length > 0 && React.createElement('div', { className: 'rounded-xl border border-white/10 bg-slate-900/60 p-3 space-y-2' },
          ...
          React.createElement('ul', { className: 'text-[11px] text-emerald-100/90 space-y-1' },
            (WRITING_TELC_TASKS[writingTelcIdx % WRITING_TELC_TASKS.length].checklist || []).map((item, i) =>
              React.createElement('li', { key: i }, `Ã¢â‚¬Â¢ ${item}`)
            )
          )
        ),
        React.createElement('div', { className: 'flex gap-2' },
    ...
    case 'guion':
      return React.createElement('div', { className: 'mb-4 rounded-xl bg-black/35 border border-rose-500/25 p-3 space-y-2' },
        React.createElement('p', { className: 'text-rose-200/90 text-sm font-bold' }, 'Escribe lÃƒÂ­neas de la historia'),
        guionLines.length > 0
      ...
      return React.createElement('div', { className: 'mb-4 rounded-xl bg-black/35 border border-cyan-500/30 p-3 flex gap-4' },
        React.createElement('div', { className: 'flex-1 space-y-3' },
          React.createElement('p', { className: 'text-cyan-200/95 text-sm font-black flex gap-2 items-center' },
            SvgIcon(SVG_PEN), ' MecanografÃƒÂ­a alemana ?'
          ),
          React.createElement('div', { className: 'space-y-2' },
            ...
            typingUseCustom && React.createElement('textarea', {
              ...
              placeholder: 'Pega aquÃƒÂ­ tu texto en alemÃƒÂ¡n...',
              className: 'w-full min-h-[140px] bg-black/45 border border-white/15 rounded-xl p-3 text-sm text-white'
            })
          ),
          React.createElement('div', {
          ...
          React.createElement('input', {
            ...
            placeholder: 'Escribe aquÃƒÂ­ exactamente el texto...',
          ...
          React.createElement('div', { className: 'flex gap-4 text-xs' },
            ...
            React.createElement('span', { className: 'text-emerald-300' }, `PrecisiÃƒÂ³n: ${typingLiveAcc}%`)
          ),
          typingFinished && typingResult && React.createElement('div', { className: 'rounded-xl bg-emerald-950/40 border border-emerald-700/40 p-3 space-y-2' },
            React.createElement('p', { className: 'text-emerald-300 font-black' }, '? Ã‚Â¡Completado!'),
            React.createElement('div', { className: 'grid grid-cols-3 gap-2 text-xs' },
              ...
              React.createElement('div', null, React.createElement('span', { className: 'text-gray-400' }, 'PrecisiÃƒÂ³n'), React.createElement('p', { className: 'text-white font-bold' }, typingResult.accuracy + '%')),
    ...
    case 'handwrite': {
      ...
      const verifyHandwrite = async () => {
        ...
        const result = await window.Muller.runOcrOnCanvas(canvasRef, viewerRef.current);
      ...
      return React.createElement('div', { className: 'mb-4 rounded-xl bg-black/35 border border-violet-500/30 p-3 space-y-3' },
        ...
        React.createElement('div', { className: 'space-y-2' },
          ...
          hwUseCustom && React.createElement('textarea', {
            ...
            placeholder: 'Pega aquÃƒÂ­ el texto a copiar...',
            className: 'w-full min-h-[140px] bg-black/45 border border-white/15 rounded-xl p-3 text-sm text-white'
          })
        ),
        ...
        React.createElement('div', { className: 'flex gap-2 items-center' },
          React.createElement('label', { className: 'text-xs flex gap-1 items-center' },
            React.createElement('input', {
              type: 'checkbox',
              checked: hwMemMode,
              onChange: e => setHwMemMode(e.target.checked)
            }),
            ' Modo memoria (oculta texto)'
          ),
          hwMemMode && !hwShowTarget && React.createElement('span', { className: 'text-[10px] text-amber-400' }, 'Texto oculto. Ã‚Â¡Escribe de memoria!')
        ),
        React.createElement('div', { className: 'flex gap-2' },
          React.createElement('button', {
            onClick: verifyHandwrite,
            className: 'px-4 py-2 bg-violet-700 hover:bg-violet-600 rounded-xl text-sm font-bold'
          }, 'Verificar con OCR'),
          React.createElement('button', {
            onClick: nextHwText,
        ...
        hwSimilarity !== null && React.createElement('div', { className: 'rounded-xl bg-black/40 border border-violet-700/40 p-3 space-y-2' },
          React.createElement('p', { className: 'text-violet-200 text-sm font-bold' },
            `Similitud: ${hwSimilarity}%`
          ),
          React.createElement('p', { className: 'text-[10px] text-gray-400' },
            'OCR: ', React.createElement('span', { className: 'text-white' }, hwOcrText || '(vacÃƒÂ­o)')
          )
        )
      );
    }
